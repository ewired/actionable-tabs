...
async function load() {
	...
	const alarm = /** @type {any} */ (
		await browser.alarms.get("moveActionableTabs")
	);
	...
	lastEl.textContent = s.lastMoveTime
		? relTime(new Date(s.lastMoveTime))
		: "Never";
	nextEl.textContent = alarm?.scheduledTime
